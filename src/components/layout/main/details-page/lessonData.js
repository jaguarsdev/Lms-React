import React from 'react';

import { Disclosure } from '@headlessui/react'

import Gettopic from './get/Gettopic';

import leftarrow from '.././../assets/svgicon/leftarrow.svg';

const LessonData = ({lessonData, courseId}) => {
    const LES_DATE = lessonData.attributes
    
    return (
        
        
        <Disclosure>
            {({ open }) => (
            <div className="flex flex-col items-start">
                <Disclosure.Button className="py-2 w-full">
                    <div className="flex items-center  w-full px-2 py-3 rounded-lg bg-gray-50 dark:bg-white/20" style={ {boxShadow: "0px 0px 15px -5px #00000090"} }>
                        <div className={`${open ? "transform -rotate-90" : ""}`}><img src={leftarrow} className="w-5 h-5 ml-1" alt="leftarrow" /></div>
                        <h2 className="font-medium">{LES_DATE.LessonTitle}</h2>
                    </div>
                        <p className={`${open ? "block p-3 text-justify border border-dashed border-gray-500 mt-2 rounded-md" : "hidden"}`} dangerouslySetInnerHTML={{ __html: LES_DATE.LessonContent }} />
                </Disclosure.Button>
                    
                <Gettopic lessonId={lessonData.id} courseId={courseId} />
            </div>
            )}
        </Disclosure>
        
    );
};

export default LessonData;