...
const Cart = ( {props} ) => {
    ...
    return (
        <div className='bg-white/80 dark:bg-white/40 w-full rounded-lg p-3 shadow-md flex items-center justify-between'>
            <img src={IMGHOST+props.CourseImage.data.attributes.formats.medium.url} alt={props.CourseTitle} className='w-1/2 md:w-1/5  rounded-lg shadow-md' />
            <div className='flex flex-col md:flex-row justify-between grow'>
                <h1 className='mx-5 font-bold'> {props.CourseTitle} </h1>
                <p className='mx-5'>{price} تومان </p>
                {
                    // isInCart(state, props.idcourse) ?
                    ...
                    // <button className="mx-5 text-white font-bold" onClick={() => dispatch({type: "ADD_ITEM", payload: props})} ><div className="bg-[#41c3d3] py-1 rounded-md  px-2 text-center font-bold w-full">موجود در سبد</div></button>
                }
            </div>
        </div>
    );
